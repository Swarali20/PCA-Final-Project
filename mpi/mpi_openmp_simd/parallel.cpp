...
int main(int argc, char** argv){
  ...
  for (int iter = 0; iter < 80; iter++){
    ...
    auto bcast_start = Clock::now();
    MPI_Bcast(&(pr[0]), num_pages, MPI_FLOAT, 0, MPI_COMM_WORLD);
    bcast_time += duration_cast<dsec>(Clock::now() - bcast_start).count();
